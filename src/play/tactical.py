...
class PrimaryIntent(str, Enum):
    ...
    KICKOFF = "kickoff"                      # 开球大力射门（开球时直接射门）
...
class DynamicTriangleCoordinator:
    ...
    def _select_primary(
        ...
        # 使用球的当前位置作为追寻目标
        point = Pose2D(context.known_ball.x, context.known_ball.y)
    ...
    def _select_primary_action(
        ...
        3. 防守 -> CLEAR 或 PRESS
        4. 争夺 -> INTERCEPT
        5. 球在边线 -> PROGRESSIVE_TOUCH（恢复位置）
        6. 射门条件满足 -> SHOOT
        7. 传球条件满足 -> PASS
        8. 压力大 -> PROGRESSIVE_TOUCH（避压）
        9. 默认 -> DRIBBLE（带球）
        ...
        # 开球：大力射门（我方开球时直接射门）
        if game.is_kickoff_for_team(self.kit.config.team_id):
            return PrimaryIntent.KICKOFF, goal
    ...
    def _primary_claim_target(
        ...
        """
        ball = context.known_ball
        # 所有意图统一使用球的当前位置作为追寻目标
        claim = Pose2D(ball.x, ball.y)
        # 计算朝向：面向动作目标
        ...
        if mode == TacticalMode.EMERGENCY_DEFEND:
            ...
            for opp in context.opponents.values():
                if opp.pose is not None:
                    dist = math.hypot(opp.pose.x - ball.x, opp.pose.y - ball.y)
                    if dist < nearest_dist:
                        nearest_dist = dist
                        nearest_opp = opp.pose
            if nearest_opp is not None:
                # 目标：离球最近的对方球员位置
                x, y = nearest_opp.x, nearest_opp.y
                # 朝向：从球朝向对方队员
                theta = math.atan2(nearest_opp.y - ball.y, nearest_opp.x - ball.x)
                return self.kit.field.clamp_inside_field(Pose2D(x, y, theta))
            # 降级：没有有效对手时回到球后方
            return Pose2D(ball.x - 1.2, ball.y)
