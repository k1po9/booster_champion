...
> English version: [Getting Started Example Strategy](https://booster.feishu.cn/wiki/CZPhwNPi7icgzEkcDGWcQwX5nIb)
> 
> 
...
示例策略是赛事官方提供的可运行Agent示例，基于[Booster Agent Framework Python API](https://booster.feishu.cn/wiki/JoYXwfPB4iTqhLkCJ0ccgSXunyd?renamingWikiNode=false)开发，符合比赛规则约束。示例策略Agent可在仿真环境中调试，参与并完成机器人3v3足球仿真赛。
...
|行为树层|behavior\_tree/|基于 py\_trees 的帧级决策：数据→守卫→战术→安全→提交|所有下层|
|策略层|play/|角色分配、Playbook 注册、踢球/走位/守门策略|所有下层|
|战术工具层|tactics/|避障、目标选择、坐标变换、踢球迟滞|所有下层|
|环境适配层|soccer\_framework/|数据类型契约、配置系统、ROS 适配器、执行命令|不依赖其他|
...
|### **main\.py是生命周期总管**<br>`main.py` 中的 `SoccerSimAgent` 是整个 Agent 的入口，也是Booster Agent Framework中最重要的类，通过两个核心钩子函数`on_agent_activated()`和`on_agent_close()`管理整个 Agent 的生命周期：<br>- **`on_agent_activated()`**<br>    在Agent启动时触发，触发时启动 ROS spin 线程，启动 runtime 控制线程，底层调用 `runtime.start()`：启动观测订阅、创建机器人控制 client，连接机器人、起控制线程。<br>- **`on_agent_close()`**<br>    当 Agent 停止时触发。调用 `runtime.stop()` 安全关闭控制循环，断开与机器人的连接。<br>> 项目根目录的`agent.toml`中，定义的`entry`默认为`"src/main.py:SoccerSimAgent"`<br>> <br>> ||
|---|---|
|### runtime\.py是总装车间<br>runtime中的`SoccerTeamRuntime`在比赛过程中持续运行，维持稳定的高频控制循环，职责包括：创建工具集、加载策略、构建行为树、管理机器人交互。<br>`WordProvider`和`GameController`为`SoccerTeamRuntime`提供了比赛的背景信息`PlayContext`。<br>`PlayContext`在行为系统`BehaviorTree`的每次策略计算中提供参考数据，受`control_loop`控制高频刷新，保证决策符合当下的比赛进程。||
...
示例策略使用了py\_trees库。py\_trees拥有成熟地行为树框架，自带tick方法，支持复杂节点，充分满足了机器人足球场景的需求。我们可以将每个决策点、行动点都抽象为行为树的节点，把复杂的行为逻辑抽象为三类节点不同的组合形式。以下是py\_trees库的三类节点：
...
> **策略层不依赖决策树框架：**PLAY 策略层没有导入 py\_trees库 中的任何类。这意味着你可以在完全不接触行为树代码的前提下，只修改 `play/` 目录下的文件来改变球队打法。行为树只是执行框架，PLAY是可灵活调整的战术内容。进一步阅读：[入门3V3示例代码中的战略战术设计](https://booster.feishu.cn/wiki/QlbzwPxtBiP12nkDXBHc5D6AnGh?from=from_copylink)
> 
> 
...
[Booster Agent Python 开发指南 v1\.7](https://booster.feishu.cn/wiki/D5Tkwm1sEiyXf0kocpdcKppOnFd)

[Booster Agent Framework Python API](https://booster.feishu.cn/wiki/JoYXwfPB4iTqhLkCJ0ccgSXunyd?renamingWikiNode=false)
