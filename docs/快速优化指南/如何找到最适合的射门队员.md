...
> English version: [How to find the most suitable shooter](https://booster.feishu.cn/wiki/I4l4wedMRiR8kAktJuwc4MH2n0g)
> 
> 

[https://www.bilibili.com/video/BV1JtMx6bEZD/?spm_id_from=888.80997.embed_other.whitelist&bvid=BV1JtMx6bEZD&vd_source=9f5e828a1512e75548042a94d63f77f3]()

找到最适合的射门队员是比赛策略最重要课题之一。示例代码策略设置一名主动进攻球员，定义为`Chaser`角色。职责是追球并完成射门。选择最适合的球员成为`Chaser`，才能最大化进攻效率。

## **追球者选择算法的思路**

追球者选择由 \`DefaultPlaybook\.select\_chaser\(\)\` 实现，每帧执行一次。它是一个**三层筛选**过程：
...
在第二层计算后，如果出现选择评分最低且差距较大，选择得分最低的机器人成为`Chaser`；如果最低得分差距 \< 0\.15m 时，取 player\_id 最小的。

这里设置平局仲裁的原因是，当两个球员评分非常接近，意味着他们和球的距离几乎一样。球的轻微移动很容易改变两个球员的大小关系。此时选 player\_id 最小的作为追球者，避免两帧之间频繁切换追球者导致行为不稳定。

[追球者选择算法演示\-可拖动球员和球\.html](图片和附件/追球者选择算法演示-可拖动球员和球.html)
...
Chaser 移动到球的过程由 `MotionController.move_to_target` 完成（详见[在3v3示例代码中优化机器人移动行为](https://booster.feishu.cn/wiki/AI5Qwwnrli5V97kCP2CcuSF8nRc)）。进入踢球范围后，Chaser 还需根据迟滞模型（`kick_hysteresis`）判断是否真正进入/退出踢球态，避免在边界附近反复进出导致行为抖动。踢球进入距离为 `soccer_kick_enter_distance`（默认 2\.5m），退出距离为 `soccer_kick_exit_distance`（默认 3\.0m），并附加 1\.5 秒的退出延迟。
